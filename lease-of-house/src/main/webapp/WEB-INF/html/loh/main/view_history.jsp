...
<body>
	...
	<div class="container">
		<div class="row">
			<form name="loh-search-form" action="<%=request.getContextPath()%>/loh/viewHistory.do" method="get">
				<div class="col-sm-12">
					<table class="loh-house-info" border="2">
						...
						<tr>
							...
							<td>
								<a href="<%=request.getContextPath()%>/loh/viewHistory.do?pageIndex=1&pageSize=${pageSize}${fn:escapeXml(queryParam)}">首页</a>
								<c:if test="${pageIndex > 1 }">
									<a href="<%=request.getContextPath()%>/loh/viewHistory.do?pageIndex=${pageIndex - 1}&pageSize=${pageSize}${fn:escapeXml(queryParam)}">上一页</a></td>
								</c:if>
							<td>
								<c:if test="${pageIndex < totalPage}">
									<a href="<%=request.getContextPath()%>/loh/viewHistory.do?pageIndex=${pageIndex + 1}&pageSize=${pageSize}${fn:escapeXml(queryParam)}">下一页</a>
								</c:if>
								<a href="<%=request.getContextPath()%>/loh/viewHistory.do?pageIndex=${totalPage}&pageSize=${pageSize}${fn:escapeXml(queryParam)}">最后一页</a>
							</td>
						</tr>
					</table>
				</div>
	...
	<!-- 引入当前页面的 js -->
	<script src="<%=request.getContextPath()%>/js/loh/main/view_history.js"></script>
</body>
</html>